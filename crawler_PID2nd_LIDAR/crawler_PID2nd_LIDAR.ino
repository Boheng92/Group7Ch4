...
double PIDthreshHoldDistance = 69.85 * 69.85;
...
void steerTheCar(double dis) {
  ...
  if (dis < threshHoldDistance * threshHoldDistance) {
    if (temp > 0.4 * threshHoldDistance * threshHoldDistance) {
      ...
      steerRight(0.5 * temp / (threshHoldDistance * threshHoldDistance) / 0.4);
    }
  } else {
      steerRight(0.9);
  } 
...
void setup()
{
  ...
  Setpoint = PIDthreshHoldDistance;
...
//            steerRight(0.1); 
          double head_dis = getHeadDis();
          double tail_dis = getTailDis();
          if(head_dis < 350 && tail_dis < 350){
             Serial.println("head_dis: " + (String)head_dis + "   tail_dis:  "+ (String)tail_dis);
              Input = calcDistance(head_dis, tail_dis);
              myPID.Compute();
